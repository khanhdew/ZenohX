...
export const PayloadEditor: React.FC<PayloadEditorProps> = ({
  ...
  const handleMinify = useCallback(() => {
    ...
    if (res.success) {
      // Remove all line breaks for compact single line
      try {
    ...
    <div className={`flex flex-col rounded-lg border bg-card text-card-foreground shadow-sm ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/40 px-3 py-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            disabled={disabled || !value}
            onClick={handleClear}
            className="inline-flex items-center gap-1 rounded-md border bg-background px-2 py-1 text-xs font-medium hover:bg-destructive/10 hover:text-destructive disabled:opacity-50 transition-colors"
            title="Clear editor"
          >
      ...
      <div className="relative flex-1">
        <textarea
          ...
          disabled={disabled}
          rows={rows}
          className="w-full resize-y bg-transparent p-3 font-mono text-xs leading-relaxed text-foreground placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
          spellCheck={false}
        />
      </div>
